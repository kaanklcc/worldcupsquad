...
export default function TransactionsPage() {
  ...
  const unlock = useCallback(async (mode: 'membership' | 'single_use', walletAddress?: string) => { setAccessLoading(true); setAccessError(null); try { const result = await payWithX402<AccessUnlockResponse>('/api/access/unlock', { mode, hasPaidX402: !accessStatus?.isDemoAccount, walletAddress: walletAddress || undefined }, createIdempotencyKey(`access-${mode}`)); setAccessStatus(result); setAccessOpen(false); await load(); } catch (unlockError) { setAccessError(unlockError instanceof Error ? unlockError.message : 'Access activation failed.'); } finally { setAccessLoading(false); } }, [accessStatus, load]);
