...
The repository includes [`render.yaml`](render.yaml) for two free Render Web Services: the FastAPI API and the Next.js frontend. Open the [one-click Render Blueprint](https://render.com/deploy?repo=https://github.com/kaanklcc/worldcupsquad), connect the repository and enter only `GEMINI_API_KEY` when prompted. Render generates a unique 256-bit `JWT_SECRET_KEY`; all non-secret production, testnet and same-origin API proxy settings are already declared in the Blueprint.

The expected public MVP is `https://wcai-web-kaanklcc.onrender.com`, with API health at `https://wcai-api-kaanklcc.onrender.com/health`. Use the frontend URL as the HackQuest MVP link after both services show **Live**. Free Render web services sleep after 15 minutes without traffic and use ephemeral local storage, so reviewers should create a new account and may need to wait briefly for the first request after idle time.
